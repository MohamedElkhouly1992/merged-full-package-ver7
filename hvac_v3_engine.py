...
def auxiliary_power_terms(bldg: BuildingSpec, cfg: HVACConfig, occ: float, deg: float) -> Dict[str, float]:
    """Return pump and auxiliary electrical power terms in kW.

    Pump power is area-normalized and scaled by operating/occupancy factor and degradation.
    Auxiliary power is area-normalized and scaled by operating/occupancy factor.
    Both are controlled by USE_HVAC_PUMPS and USE_HVAC_AUXILIARY in HVACConfig.
    """
    ...
    return {"P_pump": float(pump_kw), "P_aux": float(aux_kw)}
...
def cooling_heating_loads(bldg: BuildingSpec, cfg: HVACConfig, derived: Dict[str, float], T_mean: float, RH_mean: float, GHI_mean: float, T_sp: float, occ: float, doy: int) -> Dict[str, float]:
    ...
    return {"Q_cool_kw": q_cool, "Q_heat_kw": q_heat, "Q_HVAC_kw": q_hvac, "mode": mode, "people": n_people, "internal_kw": internal_kw, "lighting_kw": lighting_kw, "equipment_kw": equipment_kw}
...
def evaluate_controls(bldg: BuildingSpec, cfg: HVACConfig, derived: Dict[str, float], T_mean: float, RH_mean: float, GHI_mean: float, occ: float, year_frac: float, doy: int, rf: float, dust: float, T_sp: float, af: float, duration_hours: float = 24.0) -> Dict[str, float]:
    ...
    power_terms = auxiliary_power_terms(bldg, cfg, occ, deg_next)
    P_pump = power_terms["P_pump"]
    P_aux = power_terms["P_aux"]
    P_tot = P_hvac + P_fan + P_pump + P_aux
    E_period = P_tot * duration_hours
